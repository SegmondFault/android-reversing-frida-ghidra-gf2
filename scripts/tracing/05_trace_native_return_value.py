...
function install() {
  ...
  Interceptor.attach(mod.base.add(0x205f8), {
    ...
    onLeave(retval) {
      console.log("[+] leave check function retval=" + retval + " low=" + (retval.toInt32() & 0xff));
      send({type:"ret", ret: retval.toString(), low: retval.toInt32() & 0xff});
    }
  });
  ...
  Interceptor.attach(mod.base.add(0x55c3dc), {
    onEnter(args) {
      ...
      console.log("[+] at 0x55c3dc sp+13c=" + sp.add(0x13c).readU32());
    }
  });
