...
# return address after each final bl
RET_TO_SPOFF = {
    0x55bdc8: 0x15e,
    0x55bde8: 0x15d,
    0x55be08: 0x15c,
...
function install() {
  ...
  Interceptor.attach(mod.base.add(0x5862f0), {
    onEnter(args) {
      const lrOff = ptr(this.context.lr).sub(mod.base).toString(16);
      const key = "0x" + lrOff;
      this.interesting = false;

      for (const [ret, spoff] of Object.entries(retToSpoff)) {
        if (Number(ret) === parseInt(lrOff, 16)) {
    ...
    onLeave(retval) {
      ...
      console.log(
        "[XORFUNC] retaddr=0x" + this.lrOff +
        " sp+0x" + this.spoff.toString(16) +
        " a=" + this.a +
        " b=" + this.b +
        " ret=" + r +
        " xor=" + (this.a ^ this.b) +
  ...
  console.log("[+] xor callee tracer installed");
}
